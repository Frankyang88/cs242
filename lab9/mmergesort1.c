...
int merge_sort(int *a,int *w,int n, int ins){
	if(n< ins){
	insertion_sort(a,n);		
	}
	else {
	merge_sort(a,w,n/2,ins);
	if(n%2==0)
	merge_sort(a+n/2,w,n/2,ins);
	else merge_sort(a+n/2,w,n/2+1,ins);
	}
	int i;int j;int k;
	i=0;j=n/2;k=0;
	while(i<n/2 && j< n){
...
merge_sort(my_array,my_sorted_data,count,ins);
